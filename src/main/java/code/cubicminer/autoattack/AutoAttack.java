	...
	public void tick(MinecraftClient Client) 
	{
		if (keyToggleEnabled.wasPressed())
		{
			if (Client.player != null)
			{
				if (ConfigManager.getConfig().enabled)
				{
					...
					Client.player.sendMessage(Text.translatable("message.disabled", MOD_NAME).formatted(Formatting.RED), false);
				}
				else 
				{
					ConfigManager.getConfig().enabled = true;
					Client.player.sendMessage(Text.translatable("message.enabled", MOD_NAME).formatted(Formatting.GREEN), false);
				}
			}
		}
		if (Client.options.attackKey.isPressed() && Client.player != null && Client.player.getAttackCooldownProgress(0) >= 1 && ConfigManager.getConfig().enabled == true) 
		{
		...
		if (Client.player != null)
		{
			if (Client.targetedEntity != null && Client.player != null)
			{
				...
				if (Calculated <= 1.15)
					Client.player.sendMessage(Text.translatable("message.distance.in", Converted).formatted(Formatting.GOLD), true);
				else if (Calculated > 1.15 && Calculated <= 3.3)
					Client.player.sendMessage(Text.translatable("message.distance.in", Converted).formatted(Formatting.YELLOW), true);
				else if (Calculated > 3.3 && Calculated <= 7)
					Client.player.sendMessage(Text.translatable("message.distance.in", Converted).formatted(Formatting.GREEN), true);
			}
			else if (Client.player != null)
				Client.player.sendMessage(Text.translatable("message.distance.out").formatted(Formatting.RED), true);
		}
	}
}
